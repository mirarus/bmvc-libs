...
class View
{
	...
	{
		self::$engine = $data['engine'] ?: self::$engine;
		self::_data($data);
	 ...
	 * @return string
	 */
	private static function _engineBLADE(string $path = null, string $view = null, $data = null)
	{
		echo (new Blade(FS::app($path), self::_cd($path)))->make($view, (array)$data)->render();
	}
	 ...
	 * @return string
	 */
	public static function asset(string $file, string $theme = null, bool $cache = false): string
	{
		$_theme = ($theme ?: self::getData('theme')) ?: self::$theme;
		$_theme = array_key_exists($_theme, self::$themes) ? $_theme : self::$theme;
		$_themePath = FS::trim(self::$themes[$_theme]['path']);

		return url("assets/" . $_themePath . '/' . $file, false, $cache);
	}
}