...
class Filter
{
			...
			foreach ($filter as $key => $value) {
				if (!in_array($key, ($skip ?: []))) {
					if ($value != '' && !is_array($value) && !is_object($value)) {
						$filter[$key] = self::filterDataValue(self::prepareDataObject($value, ['htmlspecialchars' => [ENT_QUOTES]]));
					}
				}
			}
			return $filter;
	 ...
	 * @return array|false
	 */
	public static function filterDBM(array $filter = null, array $skip = null)
	{
		if (is_array($filter) && count($filter) > 0) {
			foreach ($filter as $key => $value) {
				if (!in_array($key, ($skip ?: []))) {
					if ($value != '' && !is_array($value) && !is_object($value)) {
						$filter[$key] = self::filterDB($value);
					}
				}
			}
			return $filter;
	 ...
	 * @return array|false
	 */
	public static function filterArray(array $arr, string $key, $val): array
	{
		return array_filter($arr, function ($value) use ($key, $val) {
